...
minp = @(H,epsilon)-epsilon*log( sum(p .* exp(-H/epsilon),1) );
minq = @(H,epsilon)-epsilon*log( sum(q .* exp(-H/epsilon),2) );
...
minp = @(H,epsilon)minp(H-min(H,[],1),epsilon) + min(H,[],1);
minq = @(H,epsilon)minq(H-min(H,[],2),epsilon) + min(H,[],2);
...
for it=1:niter
    ...
    P = p .* exp((f+g-C)/epsilon) .* q;
    % check conservation of mass
    Err(it) = norm(sum(P,1)-q,1);    
end
clf;
